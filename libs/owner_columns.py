...
class GroupbyTransformer:
    ...
    def transform(self, ds):
        ...
        if self._rules is None:
            ...
            if type(self._by_list) == str:
                if self._types == 'sum':
                    return ds[ds[self._rules_col] != self._rules_rul][[self._by_list] + self._need_columns].groupby(
                        by=[self._by_list]).sum().reset_index()

                elif self._types == 'min':
                    return ds[ds[self._rules_col] != self._rules_rul][[self._by_list] + self._need_columns].groupby(
                        by=[self._by_list]).min().reset_index()
                elif self._types == 'max':
                    return ds[ds[self._rules_col] != self._rules_rul][[self._by_list] + self._need_columns].groupby(
                        by=[self._by_list]).max().reset_index()
            else:
                if self._types == 'sum':
                    return ds[ds[self._rules_col] != self._rules_rul][[self._by_list] + self._need_columns].groupby(
                        by=self._by_list).sum().reset_index()
